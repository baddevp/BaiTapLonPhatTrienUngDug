...
public class HoaDonTraHangModelPainter extends JPanel implements ListCellRenderer<HoaDonHoanTra> {
	...
	public HoaDonTraHangModelPainter() {
...
//		soLuong.setBounds(1115, 165, 725, 35);//165
	...
	@Override
	public Component getListCellRendererComponent(JList<? extends HoaDonHoanTra> list, HoaDonHoanTra value, int index,
			boolean isSelected, boolean cellHasFocus) {
		maSach.setText("Mã trả hàng :                         " + value.getMaYeuCauTraHang());
		tenSach.setText("Nhân viên :                            " + value.getNhanVien().getTenNV());
		tacGia.setText("Khách hàng :                         " + value.getKhachHang().getTenKH());
		loaiSach.setText("Lý do trả hàng :                     " + value.getLyDoTraHang());

		soLuong.setText("Tiền trả :                             " + df.format(value.getTongHoanTra()));
		giaBan.setText("Hóa đơn trả :                      " +  value.getHoaDon().getMaHoaDon());

		ngayNhap.setText("Ngày lập :                           " + dtf.format(value.getNgayLapHoaDonTH().toLocalDate()));
		if (isSelected) {
