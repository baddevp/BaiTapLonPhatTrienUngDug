...
public class GuiTraHang extends JFrame implements ActionListener, MouseListener {
	...
	private JDateChooser dtmNgayLapTH;
	...
	public GuiTraHang(NhanVien nv) {
		...
		chkinHD = new JCheckBox("In hóa đơn");
...
//Tính tổng tiền cần thanh toán
	...
	public void xuLyThanhToan() {
        ...
        String tenNhanVien = txtTenNVTraHang.getText();
        Date ngayThanhToan = dtmNgayLapTH.getDate();
        ...
        try {
            ...
            PdfPCell cellHeadSoLuong = new PdfPCell(new Paragraph("Số lượng mua", fontHead));
            ...
            for (int i = 0; i < tblSPTra.getRowCount(); i++) {
                PdfPCell cellSP = new PdfPCell(new Paragraph(tblSPTra.getValueAt(i, 1).toString(), font));
                PdfPCell cellSL = new PdfPCell(new Paragraph(tblSPTra.getValueAt(i, 2).toString(), font));
                PdfPCell cellGia = new PdfPCell(new Paragraph(tblSPTra.getValueAt(i, 3).toString(), font));
                PdfPCell cellSoLuongTra = new PdfPCell(new Paragraph(tblSPTra.getValueAt(i, 4).toString(), font));
                PdfPCell cellThanhTien = new PdfPCell(new Paragraph(tblSPTra.getValueAt(i, 5).toString(), font));
    	    ...
    	    for (int i = 0; i < modelSPTra.getRowCount(); i++) {
    	        int soMua = (int) modelSPTra.getValueAt(i, 3);
    	        int soTra = (int) modelSPTra.getValueAt(i, 4);
    	        double giaGoc = (double) modelSPTra.getValueAt(i, 2);
    	        
    	        tongSoMua += soMua;
    	        tienDaMua += soMua * giaGoc;
    	        tongSoTra += soTra;
    	        tongTienTra += soTra * giaGoc;
