...
public class GuiQuanLyLoaiSach extends JFrame implements ActionListener, MouseListener {
	...
	public GuiQuanLyLoaiSach() {
		...
		txtMaLoaiSach.setFont(new Font("Times New Roman", Font.PLAIN, 24));
		...
		txtTenLoaiSach.setFont(new Font("Times New Roman", Font.PLAIN, 24));
		...
		pnlTacVuCon.setBounds(9, 704, 330, 140);
		...
		txtTimKiem.setBounds(10, 77, 308, 40);
		pnlTacVuCon.add(txtTimKiem);
		txtTimKiem.setColumns(10);
		
		JRadioButton radTenLoaiSach = new JRadioButton("Tên loại sách");
		radTenLoaiSach.setFont(new Font("Times New Roman", Font.PLAIN, 16));
		radTenLoaiSach.setBackground(Color.WHITE);
		radTenLoaiSach.setBounds(10, 21, 226, 23);
		pnlTacVuCon.add(radTenLoaiSach);
		
		JRadioButton radMaLoaiSach = new JRadioButton("Mã loại sách");
		radMaLoaiSach.setFont(new Font("Times New Roman", Font.PLAIN, 16));
		radMaLoaiSach.setBackground(Color.WHITE);
		radMaLoaiSach.setBounds(10, 47, 204, 23);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnThem)) {
			if (btnThem.getText().equals("Thêm")) {
				...
				txtMaLoaiSach.setText(loaiSach_dao.TuPhatSinhMaLoaiSach());
			} else {
				tblLS.addMouseListener(this);
		...
		if (o.equals(btnXoa)) {
			...
			if (btnSua.getText().equals("Sửa")) {
				int hangDuocChon = tblLS.getSelectedRow();
				if (hangDuocChon > -1) {
					dongMoNhapLieu(true);
					btnSua.setText("Hủy");
			...
			if (btnSua.getText().equals("Hủy")) {
				if (validData()) {
					...
					if (loaiSach_dao.updateLoaiSach(new LoaiSach(maLS, tenLS, moTa, vat))) {
						modelLoaiSach.setRowCount(0);
						DocDuLieuDatabase();
